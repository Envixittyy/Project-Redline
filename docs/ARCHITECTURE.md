...
The Phase 9 cloud-AI privacy architecture is implemented. A cloud request containing selected task, note, calendar, school, capture, attachment, or image data uses a two-step server flow: prepare an owner-scoped transfer manifest and canonical content digest without provider egress, then display an interactive disclosure (`AiDisclosureModal`) and consume a short-lived, one-time consent record before dispatch. Consent is bound to the owner, provider, model, purpose, exact source revisions, selected fields, and payload digest. Changing any bound value requires a new disclosure.

Phase 9 cloud adapters (Anthropic, Gemini, OpenAI) are server-only, use a fixed provider catalog and HTTPS endpoints, and return untrusted structured output for strict parsing via `AiActionProposal`. They cannot accept arbitrary base URLs, expose credentials or provider SDK objects to the browser, call application repositories as provider tools, or receive private storage URLs. Cloud-transfer consent grants disclosure once; it does not grant permission to change Forward data. Valid model actions become reviewable `operation_batches` with `source = 'ai'`, and mutations still pass fresh owner/domain validation and the separate Propose → Review → Commit boundary. `trusted_automation` remains reserved for a later bounded automation phase and does not bypass cloud consent. Full data classification, consent state, payload integrity, audit, image, failure, schema, and verification contracts are in `docs/FORWARD_ARCHITECTURE.md` section 9.

Phase 10A implements the Local AI Companion transport and security boundary (`docs/LOCAL_COMPANION_ARCHITECTURE.md`). A dedicated loopback daemon (`127.0.0.1`) mediates between Forward and local inference runtimes (**Ollama**, **llama.cpp server**, and generic **OpenAI-compatible local endpoints**). The browser never connects directly to model runtime ports. Destination addresses are strictly validated to loopback (`127.0.0.1`, `localhost`, `::1`), and requests require origin validation and an ephemeral pairing bearer token. Redline maintains an independent capability layer; local models receive zero database mutation authority and all mutating outputs flow through the immutable **Propose → Review → Apply** lifecycle.
...
Phase 9 reuses the operation-batch boundary for validated AI proposals rather than storing executable provider output. Its future migration adds owner-scoped AI preferences and metadata-only cloud transfer requests with an expiring state machine, canonical payload digest, source revision manifest, and optional resulting operation-batch relationship. Raw prompts, source content, image bytes, provider request/response bodies, and credentials are not persisted in transfer audit rows. Authenticated security-invoker operations derive the actor from `auth.uid()`, atomically grant and claim one transfer, and enforce owner equality for related operations. The outbound provider call occurs only after a successful claim; ambiguous network failures are not retried automatically.
