...
The Phase 9 cloud-AI privacy architecture gate is approved but unimplemented. A cloud request containing selected task, note, calendar, school, capture, attachment, or image data uses a two-step server flow: prepare an owner-scoped transfer manifest and canonical content digest without provider egress, then display an interactive disclosure and consume a short-lived, one-time consent record before dispatch. Consent is bound to the owner, provider, model, purpose, exact source revisions, selected fields, and payload digest. Changing any bound value requires a new disclosure. The existing `maySendToCloud` helper and a client-supplied `userConfirmed` boolean are cross-phase scaffolding, not sufficient authorization for an outbound transfer.

Phase 9 cloud adapters are server-only, use a fixed provider catalog and HTTPS endpoints, and return untrusted structured output for strict parsing. They cannot accept arbitrary base URLs, expose credentials or provider SDK objects to the browser, call application repositories as provider tools, or receive private storage URLs. Cloud-transfer consent grants disclosure once; it does not grant permission to change Forward data. Valid model actions become reviewable `operation_batches` with `source = 'ai'`, and mutations still pass fresh owner/domain validation and the separate Propose → Review → Commit boundary. `trusted_automation` remains reserved for a later bounded automation phase and does not bypass cloud consent. Full data classification, consent state, payload integrity, audit, image, failure, schema, and verification contracts are in `docs/FORWARD_ARCHITECTURE.md` section 9.
