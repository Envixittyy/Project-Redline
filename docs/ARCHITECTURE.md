...
- **Unavoidable Relay Metadata**: Supabase can observe that an owner enabled sync, random record IDs, key/format versions, per-record sync versions, per-owner cursor values, ciphertext sizes, and request/update timing. Record type, reply relationships, journal revisions, timestamps inside the payload, deletion state, and bodies remain encrypted. Pull queries request only the columns required for decryption and reconciliation.
...
- **Non-Extractable Local Key Storage**: The random master-key bytes are wrapped before they are imported as a non-extractable Web Crypto `CryptoKey` (`extractable: false`). Upon unlock, the unwrapped bytes are likewise imported non-extractably and persisted in client-side IndexedDB (`dear_dumbass_local_keys`). Passphrase byte buffers and transient raw-key buffers are cleared in `finally` blocks as promptly as JavaScript permits. Persisted keys are bound to the authenticated owner ID.
- **Authenticated Envelopes and Records**: The wrapped master key uses AES-256-GCM with a fresh 96-bit IV. Its AAD binds the envelope version, authenticated owner, key version, exact PBKDF2 parameters, and cipher parameters. Each post mutation is serialized to validated JSON, tombstone-scrubbed if deleted, and encrypted with AES-256-GCM using a new 96-bit IV. Record AAD unambiguously binds the format version, length-prefixed record ID, and key version. Unsupported versions and malformed parameters fail before expensive derivation or decryption work.
...
- **Supabase Schema & RLS**:
  ...
  - `dear_dumbass_private.change_counters`: A private per-owner monotonic cursor allocated by an owner-derived helper on every record insert and update. It prevents cross-user activity gaps from appearing in the public record cursor.
...
- **Incremental Cursor Pull**: Clients track the highest `server_change_sequence` applied locally and query only newer records via `.gt("server_change_sequence", cursor)`, minimizing bandwidth. Pulls paginate to exhaustion. A page is fully decrypted and validated before one atomic multi-store transaction applies it and advances the cursor, so a corrupt record or failed local write cannot partially import the page or skip it permanently.
...
- **Local Plaintext and Sync-Key Removal**: Journal records and conflict copies are plaintext in PrivateStore; E2EE protects the remote relay, not local data at rest. "Remove Sync Key" clears the usable in-memory key and its IndexedDB `CryptoKey`, stops background sync, and requires the passphrase to resume cloud synchronization. It does not hide or erase locally stored journal plaintext, and the UI states this explicitly.
- **Sign-Out Data Retention**: `clearSensitivePwaState()` removes the Dear Dumbass in-memory and IndexedDB sync key and resets cached repository/coordinator references. It deliberately preserves local plaintext records, sync metadata, outbox items, tombstones, and conflicts in `redline-private-store-v1` so valuable local data is not lost on logout. Cloud authorization is removed by the normal Supabase sign-out flow; a later authenticated owner must match the stored sync owner before a key can be loaded.
