...
describe("AI trust boundary in PostgreSQL (actual migrations, authenticated role)", () => {
  ...
  it("legacy V1 scoped requests cannot prepare or claim local/cloud inference", async () => {
    ...
    }))).rejects.toThrow(/school_intelligence_review_required|ai_request_unavailable/);
    await db.exec("reset role");
    ...
    await db.exec("set role authenticated");
    await expect(rpc("ai_claim_inference", proof("claim_inference", { id: attempt, payload_digest: "a".repeat(64), consent: true }))).rejects.toThrow(/school_intelligence_review_required|ai_request_unavailable/);
  });
  ...
  it("finite activated domains join the cloud registry; prediction inference remains local", async () => {
    ...
    await db.exec("reset role");
    for (const capability of ["schoolScheduleImage.propose", "blackboardCourseImage.propose", "academicCalendarImport.propose", "noteSummary.propose", "noteRewrite.propose", "noteActionItems.propose", "quickCapture.propose", "dailyPlanAdvice.propose", "courseMaterialSummary.propose", "courseMaterialStudyQuestions.propose", "contextualAssistant.propose"]) {
    ...
    for (const capability of ["schoolAssessmentPrediction.propose", "predictionTask.propose", "predictionEvent.propose", "unknown.propose"]) {
  ...
  it("prediction terminal states cannot reopen or confirm without a reviewed conversion", async () => {
    ...
    await db.exec("reset role");
    await expect(db.query("update school_assessment_predictions set status='confirmed' where id=$1", [predictionId])).rejects.toThrow(/prediction_confirmation_requires_review|prediction_terminal_state/);
    await db.query("update school_assessment_predictions set status='superseded' where id=$1", [predictionId]);
    await expect(db.query("update school_assessment_predictions set status='active' where id=$1", [predictionId])).rejects.toThrow(/prediction_confirmation_requires_review|prediction_terminal_state/);
    await db.exec("set role authenticated");
