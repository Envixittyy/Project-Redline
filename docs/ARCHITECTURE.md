...
- **Encrypted Local Backup & Restore**: Exports versioned JSON envelopes encrypted via Web Crypto API (PBKDF2 with SHA-256, 100,000 iterations, random salt, AES-GCM 256-bit, random IV). Restores decrypt in memory and validate schemas before initiating atomic IndexedDB transactions. Safe Merge preserves existing records and tombstones; Replace mode requires explicit confirmation. Zero logging of passphrases, plaintext, or decrypted content.
